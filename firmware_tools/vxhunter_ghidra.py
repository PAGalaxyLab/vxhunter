...
class VxTarget(object):
    ...
    def get_symbol_table(self):
        ...
        for i in range(self.symbol_table_start, self.symbol_table_end, self._symbol_interval):
            ...
            self.logger.debug("func_code_addr: %s" % func_code_addr)
            self._symbol_table.append({
                'func_name_addr': func_name_addr,
                'func_name_length': None,
                'func_code_addr': func_code_addr,
                'offset': i
            })
        ...
        for i in range(len(self._symbol_table) - 1):
            self._symbol_table[i]['func_name_length'] = self._symbol_table[i + 1]['func_name_addr'] - \
                                                        self._symbol_table[i]['func_name_addr']
        return True
    ...
    def _check_fix(self, func_index, str_index):
        ...
        for i in range(count):
            ...
            if i == count - 1:
                ...
                return True
            if self._string_table[str_index]['length'] == self._symbol_table[func_index]['func_name_length']:
                func_index += 1
                str_index += 1
    ...
    def find_loading_address(self):
        ...
        for func_index in range(len(self._symbol_table)):
            for str_index in range(len(self._string_table)):
                ...
                self.logger.debug(
                    "self._symbol_table[func_index]['func_name_length']: %s" % self._symbol_table[func_index][
                        'func_name_length'])
                if self._string_table[str_index]['length'] == self._symbol_table[func_index]['func_name_length']:
                    if self._check_fix(func_index, str_index) is True:
                        self.logger.debug("self._symbol_table[func_index]['func_name_addr']: %s" % self._symbol_table[func_index]['func_name_addr'])
                        self.logger.debug("self._string_table[str_index]['address']: %s" % self._string_table[str_index]['address'])
                        self.load_address = self._symbol_table[func_index]['func_name_addr'] - \
    ...
    def _check_load_address(self, address):
        ...
        for i in range(count):
            offset = self._symbol_table[i]['func_name_addr'] - address
            if offset <= 0:
                return False
